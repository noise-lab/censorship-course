...
The book's [DNS](https://en.wikipedia.org/wiki/Domain_Name_System) section makes one uncomfortable point again and again: DNS is trivially easy to manipulate (just return a different IP), and every "fix" — [DoH](https://en.wikipedia.org/wiki/DNS_over_HTTPS), [DNSSEC](https://en.wikipedia.org/wiki/Domain_Name_System_Security_Extensions), [ODoH](https://blog.cloudflare.com/oblivious-dns/) — trades one trust problem for another. These breakouts push on where you would rather have the chokepoint.
...
- DNSSEC signs answers but does not change *who* can answer. A [blockchain-based naming system](https://en.wikipedia.org/wiki/Handshake_(disambiguation)) removes the trusted root entirely — no [ICANN](https://en.wikipedia.org/wiki/ICANN), no courts. What breaks in a world where domain names cannot be seized? What gets better?
